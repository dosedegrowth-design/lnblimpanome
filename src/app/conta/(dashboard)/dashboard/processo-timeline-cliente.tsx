...
import { findEtapa, type Etapa } from "@/lib/kanban-shared";
...
export function ProcessoTimelineCliente({ etapas, eventos }: Props) {
  if (eventos.length === 0) {
  ...
  return (
    <ol className="space-y-3">
      {eventos.slice(0, 8).map((e) => {
        const etapa = e.etapa_nova ? findEtapa(etapas, e.etapa_nova) : null;
        let icon = <Settings className="size-3.5 text-gray-500" />;
        let bg = "bg-gray-100";
        if (e.tipo === "etapa") {
          icon = <ArrowRight className="size-3.5 text-emerald-600" />;
          bg = "bg-emerald-100";
            ...
            <div className="flex-1 min-w-0">
              {e.tipo === "etapa" && etapa && (
                <p className="text-sm font-semibold text-gray-900">
                  Avançou para: {etapa.nome}
                </p>
              )}
              {e.tipo === "mensagem" && (
                <p className="text-sm font-semibold text-gray-900">Mensagem da equipe</p>
              )}
              {e.tipo === "arquivo" && (
                <p className="text-sm font-semibold text-gray-900">Arquivo disponível</p>
              )}
              {e.tipo === "sistema" && e.mensagem && (
                <p className="text-sm font-semibold text-gray-900">{e.mensagem}</p>
              )}
              {e.mensagem && e.tipo !== "sistema" && (
                <p className="text-sm text-gray-700 mt-0.5 whitespace-pre-wrap">{e.mensagem}</p>
              )}
              <p className="text-[11px] text-gray-400 mt-1">{formatDateTimeBR(e.created_at)}</p>
            </div>
          </li>
        );
      })}
